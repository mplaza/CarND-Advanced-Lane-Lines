...
# instead of looping through images accept one image and return it
def process_image(img):
  ...
  # Draw the results
	...
	# make the inner part of the lane sectioned off by the right and left lane
	inner_lane_line = np.array(list(zip(np.concatenate((left_fitx, right_fitx[::-1]), axis=0), np.concatenate((yrange, yrange[::-1]), axis=0))), np.int32)

	cv2.fillPoly(lane, [left_lane_line], color=[0,0,255])
	cv2.fillPoly(lane, [right_lane_line], color=[0,255,0])
	cv2.fillPoly(lane, [inner_lane_line], color=[0,255,255])
	...
	polynomial_lane_img = cv2.addWeighted(polynomial_lane_img, 1.0, lane_warped, 0.8, 0.0)
	...
	return polynomial_lane_img


# process the video with the process_image function and output it to a new video
